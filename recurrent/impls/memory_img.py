"""
MemOCR: Per-Episode Visual Memory Rendering Agent

ALTERNATIVE IMPLEMENTATION (variant, not canonical).

This implementation uses per-episode rendering: memory is drafted in text domain at each step
AND rendered to visual memory at every step. This differs from the canonical final-only
rendering in memory_img_final_only_triple.py.

Note: The canonical paper implementation uses final-only rendering (see memory_img_final_only_triple.py).
This per-episode variant may be useful for exploration but was not the main experimental setup.

Architecture:
- Memory Drafting (text domain): Incremental updates to M_t^RT at each interaction step
- Per-Episode Rendering: Visual rendering occurs at every step (not just final)
- Each intermediate visual memory input to VL model, then merged into next step's context

References:
- See memory_img_final_only_triple.py for paper details and canonical implementation
"""
...
You are given a problem, an article, and a previous memory (the image attached).
You should first try to extract the content from the image, and then draft the memory in markdown format with the crucial information in it that helps to answer the problem.
...
TEMPLATE_FINAL_BOXED = """You are presented with a problem and a previous memory (the image attached). Please answer the problem based on the previous memory and put the answer in \\boxed{{}}.
        ...
        if active_mask.sum().item() == 0:
            ...
            for i in range(self.bsz):
                prompt_text = self.tokenizer.decode(
                    prompts[i][: self.config.max_prompt_length], skip_special_tokens=True
                )
                memory_tokens = (
                    self.memory[i][: self.config.max_memorization_length]
                    if self.memory[i] is not None
                    else self.NO_MEMORY_TOKENS
                )
            ...
            all_memory_images = batch_generate_images(all_memory_text, max_workers=int(os.cpu_count() * 0.8))
            ...
            all_memory_texts = [
                self.tokenizer.decode(c[c != self.tokenizer.pad_token_id]) if c is not None else self.NO_MEMORY_TEXT
                for c in memory_i
            ]
            ...
            all_memory_images = batch_generate_images(all_memory_texts, max_workers=int(os.cpu_count() * 0.8))
            logger.info(
                f"finished processing {len(all_memory_texts)} memory images (unique {len(unique_memory_texts)}). Time taken: {time.time() - start_time} seconds"
            )
            ...
            for prompt, memory_image, chunk_text in zip(prompt_i, all_memory_images, chunk_i):
