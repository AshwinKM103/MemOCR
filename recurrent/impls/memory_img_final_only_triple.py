"""
MemOCR: Final-Only Visual Memory Rendering Agent

CANONICAL IMPLEMENTATION for the MemOCR paper (Shi et al., 2026).

This implementation uses final-only rendering: memory is drafted in text domain at each step,
but visual rendering only occurs at the final step when generating the answer. This matches
the paper's described workflow where rich-text memory M_t^RT is iteratively updated, then
rendered to visual memory V_T only for final reading.

Architecture:
- Memory Drafting (text domain): Incremental updates to M_t^RT at each interaction step
- Memory Reading (vision domain): Single rendering at final step via lightweight renderer R
- Visual memory image then input to VL model for answer generation

This differs from memory_img.py (per-episode rendering variant) which renders at every turn.

References:
- Paper: "MemOCR: Layout-Aware Visual Memory for Efficient Long-Horizon Reasoning"
- Authors: Shi et al., Meituan (arXiv:2601.21468)
...
class MemoryDataset(RDataset):
    ...
    def __getitem__(self, item):
        ...
        row_dict["prompt_ids"] = self.tokenizer.encode(chat[0]["content"], add_special_tokens=False)
...
class MemoryAgent(RAgent):
    def __init__(self, tokenizer: PreTrainedTokenizer, config: MemoryConfig, processor: ProcessorMixin):
        ...
        self.max_input_length = (
            self.config.max_raw_input_length + self.token_message_template_length + max_image_tokens * 2
        )
        logger.info(
    ...
    def action(self) -> tuple[list[list], dict]:  # <-- Changed return type
        ...
        if active_mask.sum().item() == 0:
            ...
            logger.info(
                f"finished processing {len(all_memory_text)} memory images. Time taken: {time.time() - start_time} seconds"
            )
            ...
            self.meta_info = {
                "input_pad_to": self.max_input_length,  # Note: max_input_length is no longer defined
            ...
            logger.info("FINAL TURN: MemoryAgent.action() done")
        else:
            images = []
            ...
            chunk_i = gen_batch.batch["context_ids"][
                active_mask, self.config.chunk_size * self.step : self.config.chunk_size * (self.step + 1)
            ]
            memory_i = self.memory[active_mask]

            logger.info("Skipping memory image generation in FINAL_ONLY mode")
            ...
            for prompt, memory_text, chunk_text in zip(prompt_i, memory_i, chunk_i):
                self.messages.append(
                    self.template.format(
                        ...
                        memory=self.tokenizer.decode(
                            memory_text if memory_text is not None else self.NO_MEMORY_TOKENS, skip_special_tokens=True
                        ),
                    )
                )
            ...
            self.meta_info = {
                "input_pad_to": self.max_input_length,  # Note: max_input_length is no longer defined
                "pad_to": self.config.gen_pad_to,
                "generation_kwargs": {"max_tokens": self.config.gen_max_tokens_memorization, "n": 1},
            }
            logger.info("MemoryAgent.action() done")
    ...
    def update(self, gen_output: DataProto) -> DataProto:
        if not self.is_final:
            assert len(gen_output.batch["responses"]) == self.active_mask.sum().item(), (
                f'len(gen_output.batch["responses"]): {len(gen_output.batch["responses"])} != self.active_mask.sum().item(): {self.active_mask.sum().item()}'
            )
    ...
    def end(self, triple_answer_turn=False):
        ...
        if triple_answer_turn:
            ...
            final_mask = torch.cat(
                self.final_mask_list + [all_true_mask, all_true_mask]
            )  # repeat the last final mask because we're doing triple answer turn
            assert self.final_mask_list[-1].shape == all_true_mask.shape, (
                f"self.final_mask_list[-1] should be True x BSZ. final_mask_list[-1].shape: {self.final_mask_list[-1].shape} != all_true_mask.shape: {all_true_mask.shape}"
            )
            assert sum(self.final_mask_list[-1].long()) == sum(all_true_mask.long()), (
                f"self.final_mask_list[-1] should be True x BSZ. sum(final_mask_list[-1].long()): {sum(self.final_mask_list[-1].long())} != sum(all_true_mask.long()): {sum(all_true_mask.long())}"
            )
            vanilla_qa_mask = torch.cat(
                self.final_mask_list + [all_false_mask, all_false_mask]
            )  # .... + True * BSZ + False * BSZ + True * BSZ
            subsampled_qa_mask = torch.cat(
                self.final_mask_list[:-1] + [all_false_mask, all_true_mask, all_false_mask]
            )  # .... + False * BSZ + True * BSZ + False * BSZ
            gap_fill_mask = torch.cat(
                self.final_mask_list[:-1] + [all_false_mask, all_false_mask, all_true_mask]
            )  # .... + False * BSZ + False * BSZ + True * BSZ
            sample_index = torch.cat(self.sample_index_list + [self.sample_index_list[-1], self.sample_index_list[-1]])

            all_memory_text = [
                self.tokenizer.decode(
                    memory_tokens if memory_tokens is not None else self.NO_MEMORY_TOKENS, skip_special_tokens=True
                )
                for memory_tokens in self.memory
            ]
            all_gap_fill_qas = [
                make_gap_question_from_html(memory_md_text, is_md_input=True) for memory_md_text in all_memory_text
            ]
