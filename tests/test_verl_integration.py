...
class TestVerlConfigComposition:
    """Hydra `compose()` doubles as validation: an unresolvable interpolation
    or missing config group raises here, not at training time.
    """
...
class TestRankHelpers:
    """Single-process (non-distributed) behavior, which is what CI and this
    test suite run under. `torch.distributed` is either absent or
    uninitialized, so every helper here must degrade to rank 0 / world 1.
    """
