...
def validate_verl_config(cfg: DictConfig) -> None:
    """Fail fast if the composed config is missing required VERL bridge
    fields, before importing torch/ray or touching the filesystem.
    """
...
def _compose_native_config(native_config_name: str) -> DictConfig:
    """Fully compose VERL's native trainer config, resolving its own
    `defaults:` list (model/engine/optim sub-groups under
    verl/trainer/config/) the same way `hydra.main` would.

    `OmegaConf.load` alone is not sufficient here: `sft_trainer_engine.yaml`
    / `ppo_trainer.yaml` are themselves Hydra entrypoint configs with their
    own `defaults:` list (e.g. `model@model: hf_model`), so loading them
    with plain `OmegaConf.load` returns the *unexpanded* defaults list
    instead of the composed `model`/`engine`/`optim` sections VERL's
    trainers actually read.

    Because `scripts/train_verl.py`'s own `main()` is a `@hydra.main`
    entrypoint, `GlobalHydra` is already initialized when this runs; Hydra
    does not support nested `initialize_config_dir` contexts, so the outer
    Hydra state is cleared for this nested compose and not restored — by
    this point `cfg` has already been captured, and no code after this call
    needs the outer Hydra context again.
    """
...
def build_native_config(cfg: DictConfig) -> DictConfig:
    """Compose VERL's native trainer config and merge MemOCR's bridge fields
    on top of it. Returns a config in VERL's own schema, ready to pass
    directly to `run_sft`/`run_ppo`.
    """
...
def main(cfg: DictConfig) -> None:
    """Compose MemOCR + VERL config, wire WandB/registry tracking, and
    dispatch to the selected VERL trainer entrypoint.

    Only rank 0 drives `VERLExperimentManager` (registry writes, config
    logging); every rank still calls the VERL entrypoint so distributed
